...
The gRPC definitions in `.proto` files describe the external interface, messages, and events. From the `.proto` files, the gRPC CLI (`protoc`), generates the code framework for remote procedure calls, data access, and serialization. Take a look at xref:python:proto.adoc[Writing gRPC descriptors] for more information about defining messages and events.

TIP: Kickstart a project using the xref:python:kickstart.adoc[Python code generation tool].
...
Actions are stateless functions that can be triggered by gRPC or HTTP calls. They can also subscribe to published events, and they can send commands to other services based on the events they receive. For more information about writing actions see xref:python:actions.adoc[Implementing Actions].
...
TIP: To learn more about Akka Serverless entities see xref:python:value-entity.adoc[Implement a Value Entity] and xref:python:eventsourced.adoc[Implement an Event Sourced Entity].

To update multiple Entities from a single command use forwarding and effects. 

TIP: For more information see xref:python:forwarding.adoc[Forwarding and effects in Python].
...
xref:python:run-locally.adoc[running them locally] before deploying to Akka Serverless.
