...
It also contains the configuration to start a local Google Pub/Sub emulator that the Akka Serverless proxy will connect to when you make use of xref:python:topic-eventing.adoc[] in your service.
...
All Akka Serverless APIs are exposed as regular HTTP endpoints which you can call with `curl`. Protobuf annotations allow controlling the mapping to HTTP as discussed in xref:python:proto.adoc#_transcoding_http[Transcoding HTTP].
