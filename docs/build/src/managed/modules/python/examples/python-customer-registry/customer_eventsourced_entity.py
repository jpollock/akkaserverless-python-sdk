...
from customer_domain_pb2 import (ProductState, DESCRIPTOR as DOMAIN_DESCRIPTOR)
from customer_api_eventsourced_pb2 import (Product, _GETPRODUCTCOMMAND, _PRODUCTSERVICE, DESCRIPTOR as API_DESCRIPTOR)
#ProductAdded
...
def add(state: ProductState, command: Product, context: EventSourcedCommandContext):
    context.emit(ProductState(customer_id= command.customer_id, name=command.name))
    return Empty()
