...
def init(entity_id: str) -> CalculatorState:
    cs = CalculatorState()
    cs.value = 0
    #cs.history = []
    return cs
...
'''
@entity.command_handler("AddToValue")
def add(state: CalculatorState, command: Add, ctx: ValueEntityCommandContext):
    state.history.append(state.value)
    state.value += command.amount
    return state

@entity.command_handler("SubtractFromValue")
def subtract(state: CalculatorState, command: Subtract, ctx: ValueEntityCommandContext):
    state.history.append(state.value)
    state.value -= command.amount
    return state

@entity.command_handler("MultiplyValue")
def multiply(state: CalculatorState, command: Multiply, ctx: ValueEntityCommandContext):
    state.history.append(state.value)
    state.value *= command.amount
    return state

@entity.command_handler("DivideValue")
def divide(state: CalculatorState, command: Divide, ctx: ValueEntityCommandContext):
    state.history.append(state.value)
    state.value /= command.amount
    return state
